...
class StatisticAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if from_ is not None and to_ is not None:
            car_orders = car_orders.filter(created_at__gte=from_, created_at__lte=to_)
        ...
        cashless_payments = Balance.objects.filter(payment_type=Balance.PAYMENT_TYPE_CASHLESS)
        if from_ is not None and to_ is not None:
            cashless_payments = cashless_payments.filter(created_at__gte=from_, created_at__lte=to_)
        cashless_payments_number = cashless_payments.count()
        cashless_payments_sum = sum(payment.sum_in_jpy for payment in cashless_payments)

        # Количество отправленных контейнеров
        shipped_containers = Container.objects.filter(status=Container.STATUS_SHIPPED)
        if from_ is not None and to_ is not None:
            shipped_containers = shipped_containers.filter(created_at__gte=from_, created_at__lte=to_)
        shipped_containers_number = shipped_containers.count()
        shipped_containers_sum = sum(container.totalAmount for container in shipped_containers)

        # Количество загруженных машин в контейнеры
        loaded_cars = ContainerCar.objects.all()
        if from_ is not None and to_ is not None:
            loaded_cars = loaded_cars.filter(created_at__gte=from_, created_at__lte=to_)
        ...
        if from_ is not None and to_ is not None:
            sold_cars = sold_cars.filter(created_at__gte=from_, created_at__lte=to_)
        ...
        if from_ is not None and to_ is not None:
            car_resale = car_resale.filter(created_at__gte=from_, created_at__lte=to_)
        ...
        loaded_cars = [el['car__id'] for el in ContainerCar.objects.all().values('car__id')]
        ...
        if from_ is not None and to_ is not None:
            incomes = incomes.filter(created_at__gte=from_, created_at__lte=to_)
